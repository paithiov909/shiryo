...
:::{.aesthetic-windows-95-container-indent}
- 直近48時間の積算降水量（48時間降水量）の棒グラフ
  - 地図上で観測地点がある位置に3Dの棒グラフを描く
  - 棒は降水量で階級に分けて色をつける
  - 3Dなので、地図を傾けているとそれっぽい
:::
...
- 地図の表現には[Mapbox](https://www.mapbox.com/)というJSライブラリを使う
  - Rからは[mapboxer](https://crazycapivara.github.io/mapboxer/index.html)というパッケージとして利用できる
  - ggplot2やplotlyでも地図なら描けるが、地図のZ軸方向に棒グラフを立てる表現がとても難しい
:::
...
pre48h <- pre48h |>
  dplyr::mutate(
    ...
    colour = val2pal(val), # さっきの色分け関数
    geometry = geometry,
    .keep = "used"
  ) |>
  sf::st_as_sf() |> # `sf`に変換しなおす
  sf::st_buffer(1000) # 棒グラフになるPOLYGONをつくる
```
...
この地図は左クリックでのドラッグでパン、右クリックでのドラッグで回転、ホイールで倍率の変更ができる
...
Mapboxには、地図にグラフを重ねることができる特別な機能があるわけではないが、[fill-extrusion](https://docs.mapbox.com/style-spec/reference/layers/#fill-extrusion)というレイヤーとして追加することで、高さがある地物を描画することができる
...
:::{.aesthetic-windows-95-container-indent}
- ただし、力技による
  - Mapboxの`fill-extrusion`を使えば、高さのある地物を表現できる
- 3D棒グラフ＋地図の表現はかなり大変
- そもそもこれが見やすい、便利な表現なのかイマイチわからない
- でも、変な図を描くと[R]{.aesthetic-pepsi-red-color}力の高まりを感じられる
:::
