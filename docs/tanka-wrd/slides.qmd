...
- Rパッケージをつくったりしている<br />[gibasa](https://github.com/paithiov909/gibasa/), [audubon](https://github.com/paithiov909/audubon/), [apportita](https://github.com/paithiov909/apportita/)
...
- 前後2つの文（単語列）のあいだの意味的な隔たりを定量的に評価できたらうれしい
  - 単語列間の非類似度（距離）を計算すればよさそう
  - 非類似度を計算するために、まず、単語列をその意味が反映されたベクトル表現にすることを考える
...
## Word Rotator's Distance {auto-animate="true"}

- 2つの文ベクトル（単語ベクトルを縦に積み上げたもの）のあいだの最適輸送を考える
  - 各単語の重み（確率変量）として、各単語ベクトルのL2ノルムを正規化したものを使う
...
## WRDを計算してみる{auto-animate="true"}

全角スペースで区切る場合、五島の短歌のほうが前後の非類似度が大きい
...
一方で、こういうのだと、区切る位置によってWRDの値が変わってしまう。どうすればいい？
...
##### 前後ともに5文字以上のペアだけにしぼった場合{class="pt-8 pl-3"}
...
体言止めの短歌は前後のWRDが大きくなりやすい傾向がありそう？
<!-- TODO: グラフ -->

## 前後のWRDが小さい短歌

同じような語句を繰り返し用いている短歌では、前後のWRDが小さくなりやすい
<!-- TODO: グラフ -->
...
:::{.incremental}
- 短歌の「詩的度」の一側面を測れそうな尺度として、短歌を前後2つに機械的に区切った「文」のあいだのWRDを計算した
- 評価用データセットのようなものはないので評価できないが、感覚的には悪くない尺度に思われた
