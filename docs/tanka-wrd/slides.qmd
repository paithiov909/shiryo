...
- ***「詩的」に感じられる「飛躍がある」とは、どのようなことなのだろうか？***
...
- それでも、どちらがよりよい短歌だろうかと考えたくなるのも、人のこころというもの。ここでは、2つの短歌を見せられたとして、どちらがより短歌として詩的か、「ポエム力・詩的度が高いか」をどう判断するかを考えてみたい
...
:::{.notes}
- 注意点として、今回紹介する文の非類似度を計算する方法は、やり方としてはやや古い
- 自然言語処理の技術の進展するスピードは凄まじい。簡単にやるなら、PythonからOpenAIのAPIを叩いて、文の埋め込みを直接得てしまって、コサイン類似度を計算すれば実現できそう
- ただし、後で紹介する例だが、今回は全部で7万首程度、あわせて43万個の文のペアについて文ベクトルを取得したかったので、そういうやり方は避けたかった
- あえて古いやり方でやっている点に注意
:::
...
:::{.notes}
- イメージとしては、文Aの確率変量（ベクトル）に適当な割合をかけながら、文Bの確率変量と同じになるように移し替えるみたいなことを考える。WRDの場合、これは文Aの重みを文Bの重みに適当に移し替えることで、単語を超球面上で回転させることに相当する
...
- すこし詳しく見ると、たとえば、ふたつの文s1, s2の文ベクトルがそれぞれ6行6列（6次元で6単語）と4行6列（6次元4単語）で与えられていたとして、コサイン距離はこんな感じで計算できる
...
- ある程度はどうしようもないが、前後の文のどちらかが極端に短いペアを取り除いてから集計すると、よりそれっぽい代表値が得られる
:::
